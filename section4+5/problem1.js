...
//This problem is in secion 4 lecture 5 of the course.
...
//the above solution uses nested looping which should be generally avoided, because of time complexity. It returns Big O value of O(n^2). Avoid nested looping in coding.
...
function checkCommonItem2(arr1, arr2) {
  //step1: convert the array1 into an object which contains the elements of the array as its properties:
  const map = {};
  for (let i = 0; i < arr1.length; i++) {
    if (!map[arr1[i]]) {
      const item = arr1[i];
  ...
  //step2: compare the elements of the array2 with each element of the array1:
